...
func newGormSQL(dbType string, option SQLOption) GormSQL {
	...
	sqlInst.read = getGormSQL(dbType, DBAccessRead, option)
	sqlInst.write = getGormSQL(dbType, DBAccessWrite, option)
	return sqlInst
}
...
func getGormSQL(dbType string, dbAccessType string, option SQLOption) *gorm.DB {
